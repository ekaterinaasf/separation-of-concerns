...
var importFresh = function importFresh(moduleId) {
  ...
  if (eval('require').cache[filePath] && eval('require').cache[filePath].parent) {
    var i = eval('require').cache[filePath].parent.children.length;

    while (i--) {
      if (eval('require').cache[filePath].parent.children[i].id === filePath) {
        eval('require').cache[filePath].parent.children.splice(i, 1);
      }
    }
  } // Delete module from cache


  delete eval('require').cache[filePath]; // Return fresh module
